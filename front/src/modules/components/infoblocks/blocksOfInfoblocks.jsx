    ...
    let blocksOfInfoblocks =
        props.allBlocksFromState.map(block => {
            ...
            const infoblocksCreator = (number) => {
                ...
                return (
                    <div key={`blocks-${block.arrayForRender[number].nameOfType}-${number}`} className="items">
                        <h2 key={`block-h2-${number}`} className="title">{block.arrayForRender[number].nameOfType}</h2>
            ...
            if (block.typeOfAction === CHANGE_CURRENT_INFOBLOCKS) {
                ...
                items =
                    <div key={`blocks-${block.arrayForRender.nameOfType}`} className="items">
                        <h2 key={`block-h2-${block.arrayForRender.nameOfType}`} className="title">{block.arrayForRender.nameOfType}</h2>
